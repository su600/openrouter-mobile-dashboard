...
- 🧊 **毛玻璃视觉效果**：参考蓝色半透明桌面界面，为顶部栏、看板卡片、新品栏、弹窗和登录框加入高透明度蓝色玻璃层、强背景模糊、环境光晕、细描边与内沿高光，并适配深浅主题；不支持 `backdrop-filter` 的浏览器自动回退为不透明底色
...
### 2026-09-26
- 🧊 参考蓝色半透明桌面界面进一步增强毛玻璃：降低深色面板不透明度、增大背景模糊并加强蓝色边缘高光，同时保留浅色主题及不支持模糊效果时的纯色降级
