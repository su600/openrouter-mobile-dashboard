...
- 🧊 **毛玻璃视觉效果**：参考深色蓝灰设置面板，为顶部栏、看板卡片、新品栏、弹窗和登录框加入半透明模糊层、细描边、内沿高光与圆角，并适配深浅主题；不支持 `backdrop-filter` 的浏览器自动回退为不透明底色
...
### 2026-09-26
- 🧊 参考深色蓝灰设置面板细化看板毛玻璃风格：半透明磨砂面板、细描边、内沿高光、较大圆角和蓝色标题，并保留浅色主题及不支持模糊效果时的纯色降级
