...
- 🗓️ **本月累计精准计算**：`/activity` 接口同样存在 1~2 天延迟，若直接聚合会导致「本月累计」滞后、甚至出现「本月累计 < 今日消费」的倒挂。现改为：以每日 0 点基准的滚动历史逐日精确重建（相邻两连续自然日基准之差 = 前一日消费），`/activity` 兜底，今日用实时值覆盖，并兜底保证 `本月累计 ≥ 今日消费`（已修复）
