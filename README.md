...
- 🧊 **毛玻璃视觉效果**：顶部栏、看板卡片、新品栏、弹窗和登录框采用高透明度炭灰／蓝灰玻璃层、强背景模糊、细描边与内沿高光，辅以克制的蓝色点缀，并适配深浅主题；不支持 `backdrop-filter` 的浏览器自动回退为不透明底色
...
### 2026-09-26
- 🧊 毛玻璃效果保留 1% 透明底色与强背景模糊，深色配色从浓蓝恢复为炭灰／蓝灰，仅以蓝色作轻量强调，并保留浅色主题及不支持模糊效果时的纯色降级
