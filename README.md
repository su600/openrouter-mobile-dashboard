...
- 💰 **旗舰模型价格对比**：看板底部新增两列卡片，并排对比 **GPT 家族** 与 **Claude 家族**「最新一代」旗舰模型价格（输入/输出，单位 USD / 百万 tokens，跟随币种切换；自动排除 `:batch` 与别名变体，按价格高→低排列，最贵者标注「旗舰」徽标），数据来自 OpenRouter 官方模型列表接口（1 小时缓存）
