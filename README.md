...
- 🧊 **毛玻璃视觉效果**：参考蓝色半透明桌面界面，为顶部栏、看板卡片、新品栏、弹窗和登录框加入蓝色磨砂层、环境光晕、细描边与内沿高光，并适配深浅主题；不支持 `backdrop-filter` 的浏览器自动回退为不透明底色
...
### 2026-09-26
- 🧊 参考蓝色半透明桌面界面增强毛玻璃风格：加入蓝色环境光晕、冷色磨砂面板、柔和内沿高光与蓝色标题，并保留浅色主题及不支持模糊效果时的纯色降级
