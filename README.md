...
- 🆕 **新品新闻滚动条**：账户概览下方自动无缝滚动展示 Claude / GPT / DeepSeek / Gemini / Llama / Qwen / Mistral 等主流厂商最新发布的模型及发布日期，数据来自 OpenRouter 官方模型列表接口（12 小时缓存）；自动去除模型名称中重复的厂商前缀，近 7 天发布的模型在厂商名前加 ⚡、模型名称以绿色突出；支持触屏滑动、触控板滚动和鼠标拖动查看，交互后自动滚动会短暂停顿再继续
- 📈 **消费趋势图**：近 30 天每日消费折线图
