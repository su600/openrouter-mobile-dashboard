...
- 📈 **消费趋势图**：近 30 天每日消费曲线图（单调三次插值平滑，避免直线折角与过冲），默认在曲线上直接标注数值（字号 16px，自适应减少密集标签，并始终标注最高值与最近一天）；明确设置曲线点与坐标轴样式，避免异常深色线条
...
- 🧊 **毛玻璃视觉效果**：参考蓝色半透明桌面界面，为顶部栏、看板卡片、新品栏、弹窗和登录框加入高透明度深蓝玻璃层、强背景模糊、蓝色环境光晕、细描边与内沿高光，并适配深浅主题；不支持 `backdrop-filter` 的浏览器自动回退为不透明底色
...
### 2026-09-26
- 🧊 参考蓝色半透明桌面界面进一步增强毛玻璃：降低深色面板底色不透明度至 1%、加大背景模糊并保持深蓝底色与蓝色边缘高光，同时保留浅色主题及不支持模糊效果时的纯色降级

### 2026-09-25
- 📈 近 30 天消费趋势图使用单调三次插值平滑曲线，避免直线折角与过冲；数值标注字号增至 16px，并自适应避免拥挤、始终标注最高值与最近一天；明确曲线点和坐标轴样式以避免深色边线
